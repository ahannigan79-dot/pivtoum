...
// The MDX prose is now HAND-MAINTAINED in two voices (career + studying), so we
// no longer overwrite an existing <slug>.mdx from source — that would revert the
// reframe. careers.ts (the scores) is still regenerated from source every run;
// a brand-new sampler still gets a first MDX draft to hand-edit from.
const keptMdx = [];
for (const file of files) {
  const slug = file.replace("-sampler-free.md", "");
  const md = readFileSync(join(SRC, file), "utf8");
  parsed.push(parseSampler(slug, md));
  const dest = join(MDX_DIR, `${slug}.mdx`);
  if (existsSync(dest)) keptMdx.push(slug);
  else writeFileSync(dest, generateMdx(slug, md));
}
if (keptMdx.length) warn("mdx", `kept ${keptMdx.length} hand-maintained MDX file(s) — not overwritten from source`);
...
// Studying-voice companions are hand-authored as content/careers/<slug>-studying.mdx.
// Pick up whichever exist so the registry stays in sync with the files on disk.
const studyingSlugs = parsed
  .map((c) => c.slug)
  .filter((slug) => existsSync(join(MDX_DIR, `${slug}-studying.mdx`)));
...
${studyingSlugs.map((slug) => `  "${slug}": () => import("./${slug}-studying.mdx"),`).join("\n")}
};

**The short answer, if you're weighing whether to study data science: this is not the safe technical career it was sold as three years ago.** The people *building* AI systems are reasonably protected. The much larger number of people who query databases, build dashboards and produce analyses are not, and that is where almost all the entry-level jobs are.
...
Inputs arrive as structured data. Outputs are code, charts and text. There is no physical component, no licensing requirement, and no regulatory body standing between the work and its automation. That's the gap you're planning a route across: the work you'd start on is the exposed part, and the work you're aiming for is the protected part.
...
There is an intuition worth dismantling here, because a lot of people choosing this path are relying on it: that working *with* AI must be safer than working alongside it. That people who understand these systems will be the last displaced by them.
...
Which versions hold up — and which point straight at the automated end — is the whole question, and it turns on how a program is built rather than on the score. The destination to aim at is the judgment-and-accountability end; the on-ramp you start on is the exposed one, and a lot of degrees stop there. Inside the community, the Learn library covers which versions of the path actually hold up, the routes into the protected end, and how to judge where a program is really pointed.

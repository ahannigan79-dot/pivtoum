**The short answer, if you work in data science: this is not the safe technical career it was sold as three years ago.** The people *building* AI systems are reasonably protected. The much larger number of people who query databases, build dashboards and produce analyses are not, and that is where almost all the work sits.
...
Inputs arrive as structured data. Outputs are code, charts and text. There is no physical component, no licensing requirement, and no regulatory body standing between the work and its automation. Where your day-to-day work sits on the line between answering questions and owning recommendations is what your own exposure turns on.
...
There is an intuition worth dismantling here, because a lot of people in this field are relying on it: that working *with* AI must be safer than working alongside it. That people who understand these systems will be the last displaced by them.
...
The move the score points at is toward the end where judgment and accountability live — building systems rather than using them, framing problems rather than answering them, owning recommendations rather than producing requested outputs. There is no protected core to retreat into here, so the only security on offer is seniority, and closing the distance to it is the single highest-leverage thing you can do.

If your day is mostly writing SQL, building dashboards and turning requests into analyses, that's the layer moving fastest, and understanding the systems doesn't protect you from them. What separates the exposed tier from the durable one is whether a human must answer for the output — the surprising result you had to interpret, the recommendation that cost money, the call about what *shouldn't* be automated.

Which versions of the work hold up — and which point straight at the automated end — is the whole question, and it turns on the shape of the work rather than the score. Inside the community, the Learn library covers which parts of the field actually hold up, the routes into the protected end, and how to judge where your own role is really pointed.
...
## What's in your data science AI Exposure Report
This report tells you where data science stands. The community is how you act on it.
